...
- Write script to Ingest changes incrementally
...
- Top trending videos
	- based on user interactions: 
		...
		- comments
		- likes
	- NOT the most-viewed overall for the year
...
- This python script uses 'awswrangler' and creates a 'glue catalog', acting sort of like a glue crawler but in a lambda function.
...
**Why only do US_category_id.json?**
...
#### Architecture
![[Pasted image 20220613223825.png]]
...
### Next Steps
- Added glue job 'reduced_metadata' to make a new file with reduced redundancy
- Added crawler to catalog this new data

```powershell
aws glue create-crawler `
	--name "krozic-youtube-clean-reduced-metadata" `
	--database-name "krozic_youtube_clean" `
	--role "krozic-youtube-glue-s3-role" `
	--targets '{\"S3Targets\": [{\"Path\": \"s3://krozic-youtube-clean-useast1-dev/youtube/reduced_metadata\"}]}'
```